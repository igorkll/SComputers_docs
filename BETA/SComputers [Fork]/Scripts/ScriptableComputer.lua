...
function ScriptableComputer:sv_yield()
    ...
    if execTime > maxcputime then
        if self.longExecution >= 3 or execTime > maxcputime * 3 then
...
function ScriptableComputer.client_onFixedUpdate(self, dt)
    ...
    if self.aiGen then
        ...
        if str then
            ...
            self.gui:setText("ScriptData", formatBeforeGui(str))
...
function ScriptableComputer.cl_onSaveScript(self)
    ...
    if #self.last_script > ScriptableComputer.maxcodesize then
        self:cl_internal_alertMessage("the maximum code size is 32KB")
